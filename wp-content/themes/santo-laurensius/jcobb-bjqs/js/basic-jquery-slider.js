    ...
    $.fn.bjqs = function (options) {
        ...
        if (settings.showControls && slideCount > 1) {
            ...
            var $controlContainer = $('<ul class="bjqs-controls"></ul>'),
                $next = $('<li><a href="#" class="bjqs-next" class="controls"><strong>' + settings.nextText + '</strong></a></li>'),
                $previous = $('<li><a href="#" class="bjqs-prev" class="controls"><strong>' + settings.prevText + '</strong></a></li>');
            ...
            if (settings.centerControls) {
                ...
                $next.children('a').css('top', offset);
                $next.children('a').css('position', 'absolute').show();
                $previous.children('a').css('top', offset);
                $next.children('a').css('position', 'absolute').show();
        ...
        if (settings.useCaptions) {
            ...
            $.each(slides, function (key, value) {
                ...
                var $slide = $(value);
                var $slideChild_temp = $slide.children('a');
                var $slideChild = $slideChild_temp.children('img:first-child');
                var title = $slideChild.attr('title');
				var imglink = $slideChild_temp.attr('href');
                if (title) {
                    var $caption = $('<p onclick = "window.location.href = \'' + imglink + '\'" class="bjqs-caption">' + title + '</p>');
        ...
        var bjqsGo = function (direction, position) {
                ...
                if (!animating) {
                    ...
                    } else if (settings.animation === 'slide' && slideCount > 1) {
