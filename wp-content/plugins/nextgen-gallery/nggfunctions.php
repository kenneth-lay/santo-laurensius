...
function nggShowGallery( $galleryID, $template = '', $images = false ) {
    ...
    if ( !$picturelist )
        return __('<p>Maaf, kami belum mengisi gambar pada album ini.</p>','nggallery');
...
function nggCreateGallery($picturelist, $galleryID = false, $template = '', $images = false) {
    ...
    $thumbheight = $ngg_options['thumbheight'];
		...
		$gal_count2 = $gal_count;
		$gal_count3 = $gal_count;
		$set_max = 0;
		for ($i = 1; $i < $originalMaxElement + $gal_count + 1; $i = $i + 1)
		{	
			$old_gal_count = $gal_count;
			$gal_count = $gal_count - $originalMaxElement;
			if ($old_gal_count > 0 AND $gal_count <= 0)
			{	
				$set_max = $i + 1;
				break;
			}
		...
		$nggpage = 1;
		$current_offset = 0;
		for ($i = 1; $i < $suspected; $i = $i + 1)
		{	
			$gal_count2 = $gal_count2 - $originalMaxElement;
			if ($gal_count2 < $originalMaxElement)
			{	
				$nggpage = $suspected;
			}
		}
		if ($gal_count < 0)
		{
			$img_count = $gal_count * -1;
			$gal_count = 0;
		}
		//echo $gal_count;
		//$maxElement = $maxElement - $gal_count + $img_count;
    	//echo $nggpage;
    	if ($set_max > 0)
    	{
    		for ($i = $set_max - 1; $i < $suspected; $i = $i + 1)
    		{
    			if ($i < $set_max)
    			{
    				$current_offset = $img_count;
    			}
    			else
    			{
    				$current_offset = $current_offset + $originalMaxElement;
    			}
    		}
    	}
    	else
    	...
    	if ($nggpage < $set_max)
    	{
    		$maxElement = $img_count;
    	}
    	else
    	{	
    		$maxElement = $originalMaxElement;
    	}
    	//echo $maxElement;
    ...
    if ($maxElement > 0 || $maxElement == $img_count) {
    	/*
        ...
        */
        ...
        $total = count($picturelist) + $gal_count3;
        //echo $total;
        ...
        if (!$ngg_options['galHiddenImg']){
	        ...
	        else if ($nggpage < $set_max - 1)
	        {
	        	array_splice($picturelist, 0, 100000000000);
	        }
        ...
        $navigation = $nggNav->create_navigation($page, $total, $originalMaxElement);
    } else {
