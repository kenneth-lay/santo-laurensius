		...
		if ($maxElement > 0) {
			...
			if ( $total > $maxElement ) {
				...
				if ( 1 < $page ) {
					...
					$r .=  '<a class="prev" id="ngg-prev-' . $previous . '" href="' . $this->prev . '">' . '&larr; Halaman sebelumnya</a>';
				}
				...
				if ( ( $page ) * $maxElement < $total || -1 == $total ) {
					...
					$r .=  '<a class="next" id="ngg-next-' . $args['nggpage'] . '" href="' . $this->next . '">' . 'Halaman selanjutnya &rarr;</a>';
				}
				
				$this->output = "
				<div class='ngg-navigation' style = 'padding-top: 30px;'>
					<div class = 'previous-next span-17 last span-18_second-column fill-grey-light' style= 'width: 660px; border-top: 1px dotted #CC3300; border-bottom: 3px solid #CC3300;'>
						$r
					</div>
				</div>";
			} else {
