...
static function FileBrowserList(&$content, &$parents, $cat_tpl, $file_tpl, $root_cat=null)
{
	...
	foreach($cats as $cat) {
		...
		$content .= '<span>' . $cat->GenTpl($cat_tpl, 'ajax'). '</span>' . '<div class = "cat-desc">' . $cat->cat_description . '</div>';

		if($has_children) {
			$content .= "<ul>\n";			
			if($open)
			{
				self::FileBrowserList($content, $parents, $cat_tpl, $file_tpl, $cat);
			}
			else 
			{
	...
	foreach($files as $file)
			$content .= '<li id="wpfb-file-'.$file->file_id.'"><span>'.$file->GenTpl($file_tpl, 'ajax').'</span><div class = "file-desc">' . $file->file_description . '</div></li>';
}
