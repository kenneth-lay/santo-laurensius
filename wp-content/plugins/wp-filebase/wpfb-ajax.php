...
switch ( $action = $_REQUEST['action'] ) {
	...
	case 'tree':
			...
			if($c->CurUserCanAccess())
				$children[$i++] = array('id'=>sprintf($cat_id_format, $c->cat_id),
					'text'=> $catsel ?
								   ...
								   :($filesel ? (esc_html($c->cat_name)." ($c->cat_num_files / $c->cat_num_files_total)") : $c->GenTpl($cat_tpl, 'ajax') . sprintf('<div class = "cat-desc">' . $c->cat_description . '</div>')),
		...
		if((empty($_REQUEST['cats_only']) || $_REQUEST['cats_only'] == 'false') && !$catsel) {
			...
			foreach($files as $f)
				$children[$i++] = array('id'=>sprintf($file_id_format, $f->file_id), 'text'=>$filesel?('<a style = href="javascript:'.sprintf($onselect,$f->file_id,str_replace('\'','\\\'',htmlspecialchars(stripslashes($f->file_display_name)))).'">'.esc_html($f->GetTitle(24)).'</a> <span style="font-size:75%;vertical-align:top;">'.esc_html($f->file_name).'</span>'):($f->GenTpl($file_tpl, 'ajax') . sprintf('<div class = "file-desc">' . $f->file_description . '</div>')), 'classes'=>$filesel?'file':null);
		}
