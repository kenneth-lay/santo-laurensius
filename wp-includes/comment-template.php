...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Nama (lengkap):' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
		'email'  => '<p class="comment-form-email"><label for="email">' . __( 'Alamat e-mail:' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
		            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
		'url'    => '<p class="comment-form-url"><label for="url">' . __( 'Alamat situs web Anda:' ) . '</label>' .
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'Anda harus <a href="%s">log in</a> sebelum memberi komentar' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Anda akan memberi komentar atas nama akun: <a id = "nama-akun" href="%1$s">%2$s</a>.<br /> Atau anda mau <a id = "link-keluar" href="%3$s" title="Log out dari akun ini"><i>log out</i></a> ?' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes">' . __( 'Selain konten komentar, hanya nama dan alamat e-mail yang wajib Anda isi. Alamat e-mail anda tidak akan kami tampilkan pada publik.<div class = "prepend-top-negative-small">&nbsp;</div>' ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '<p class="form-allowed-tags">' . sprintf( __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Tinggalkan "jejak" anda dengan memberi komentar pada entri ini:' ),
	...
	?>
		<?php if ( comments_open( $post_id ) ) : ?>
			...
			<div id="respond">
				<div id = "reply-title" class="colsubtitle" style = "width: 680px;"><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?> </div>
				<br />
				<div class = "prepend-left-2">
